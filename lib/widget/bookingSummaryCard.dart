...
class _BookingSummaryCardState extends State<BookingSummaryCard> {
  ...
  Future<void> GetAddressFromLatLong(GeoPoint locat) async {
    List<Placemark> placemarks =
        await placemarkFromCoordinates(locat.latitude, locat.longitude);
    print(placemarks);
  ...
  accept() {
    updateBooking(widget.booking.docid, 'Accepted').then((value) {
      setState(() {
  ...
  reject() {
    updateBooking(widget.booking.docid, 'Rejected').then((value) {
      setState(() {
        widget.booking.status = 'Rejected';
        widget.booking.statusOn = DateTime.now();
        buttonVisible = false;
  ...
  completed() {
    updateBooking(widget.booking.docid, 'Completed').then((value) {
      setState(() {
                          ...
                          child: Column(children: [
                            ...
                            Text(
                              DateFormat.yMMMMd('en_US')
                                  .format(widget.booking.statusOn),
                              overflow: TextOverflow.ellipsis,
                              style: TextStyle(
                                fontSize: 12,
          ...
          Expanded(
            child: Align(
              ...
              child: IconButton(
                ...
                icon: Icon(Icons.phone, size: 23.0, color: Color(0xff7b4949)),
                //label: Text('Home')
              ),
            ),
          ),
  ...
  Widget buildText(BuildContext context) => Theme(
      ...
      */
          ...
          children: [
            ...
            Divider(color: Color(0xff7b4949)),
            GestureDetector(
              onTap: () {
